...
def export_osm_buildings(city="Montreal", output="buildings_cache.geojson", d=0.045):
...
if __name__ == '__main__':
    ...
    if args.AskCity:
        ...
        root = tk.Tk()
        root.withdraw()
        city = askstring("Ville", "Entrez le nom de la ville à afficher :", initialvalue=city)
        if not city:
            sys.exit(0)
    ...
    if should_extract:
        if OVERPY_AVAILABLE:
            print("Extraction du cache des buildings OSM…")
            try:
                export_osm_buildings(city=city, output=geojson_cache_path, d=d_box)
            except Exception as e:
                print(f"Échec de l'extraction OSM : {e}")
        else:
            print("AVERTISSEMENT: Extraction ignorée car le module 'overpy' n'est pas installé.")
    if not os.path.exists(geojson_cache_path) or os.stat(geojson_cache_path).st_size == 0:
        print(f"Création d'un fichier cache GeoJSON vide : {geojson_cache_path}.")
        with open(geojson_cache_path, "w", encoding="utf-8") as f:
            json.dump({"type": "FeatureCollection", "features": []}, f)

    print(f"Cache OSM trouvé : {geojson_cache_path}. Préparation du chargement par le navigateur.")
    ...
    print(f"Fichier HTML temporaire créé : {html_temp_path}.")

    try:
        os.chdir(args.Path)
        print(f"Changement du répertoire de travail vers: {os.getcwd()}")
        webview.create_window(
            f"Carte 3D MapTiler/OSM + Terrain Satellite – {city}",
            url=html_temp_path,
            width=1200,
            height=890
        )
        ...
        print(f"Erreur lors du démarrage de webview : {e}")
